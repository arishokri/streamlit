...
def create_links_list(links_string):
    links_list = []
    errors_list = []
...
def store_links():
    db_collection = st.session_state.vector_db_collection
    links_list = create_links_list(st.session_state.links_text)
    if links_list:
        web_loader = WebBaseLoader(web_paths=links_list)
        docs = web_loader.load()
        for doc in reversed(docs):
            if db_collection.get(where={"source": doc.metadata["source"]})["documents"]:
                st.error(
                ...
                st.info(
                    body=f"Document {doc.metadata['source']} was successfully loaded."
                )
        if docs:
...
st.radio(
    ...
    options=["Default", "Tables-Focused"],
    key="pdf_mode",
    horizontal=False,
...
def store_pdfs():
    ...
    if files := uploaded_files:
        ...
        for file in files:
            ...
            split_and_store(docs)
            st.info(body=f"Document {file.name} was successfully loaded.")
    else:
...
with st.form(key="links_form", clear_on_submit=True):
    ...
    st.form_submit_button(label="Import Links", on_click=store_links)
